...
const MAX_BODY_BYTES = process.env.MAX_BODY_BYTES
...
function callChatGPT(messages, callback) {
  if (DEBUG_MOCK) {
    ...
    return process.nextTick(() =>
      callback(null, `모의응답: ${lastUser ? lastUser.content : ""}`),
    );
  }
  ...
  const req = https.request(options, (res) => {
    ...
    res.on("end", () => {
      if (res.statusCode < 200 || res.statusCode >= 300) {
        return callback(
          new Error(`OpenAI API error ${res.statusCode}: ${body}`),
        );
      }
      try {
...
const server = http.createServer((req, res) => {
  ...
  try {
        ...
        try {
          ...
          if (pathname === "/chat") {
            ...
            const messages = [
              {
                role: "system",
                content: CUSTOM_PROMPT || "기본 프롬프트 내용",
              },
            ];
            recentPairs.forEach((pair) => {
              if (pair.question)
                messages.push({ role: "user", content: String(pair.question) });
              if (pair.answer)
                messages.push({
                  role: "assistant",
                  content: String(pair.answer),
                });
            });
              ...
              if (err) {
                ...
                return jsonRes(
                  res,
                  {
                    success: false,
                    msg: "AI 호출 실패",
                    details: DEBUG_MOCK ? String(err) : undefined,
                  },
                  500,
                );
              }
          ...
          return jsonRes(
            res,
            {
              success: false,
              msg: "서버 오류",
              details: DEBUG_MOCK ? String(e) : undefined,
            },
            500,
          );
        }
    ...
    const target =
      pathname === "/" ? "login.html" : pathname.replace(/^\/+/, "");
    ...
    try {
      jsonRes(res, { success: false, msg: "서버 처리 중 오류 발생" }, 500);
    } catch (e) {}
  } finally {
